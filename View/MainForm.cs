...
namespace CompilerV2.View
{
    public partial class MainForm : Form, IMain
    {
        ...
        private TabPage polizTabPage;
        private DataGridView polizLexemsGrid;
        private DataGridView polizTetradsGrid;
        private RichTextBox polizSummaryRichTextBox;
        ...
        private void InitializePolizTab()
        {
            polizTabPage = new TabPage("ПОЛИЗ");
            ...
            tablesSplit.SplitterDistance = Math.Max(100, tablesSplit.Width / 2);

            polizLexemsGrid = new DataGridView
            {
                Dock = DockStyle.Fill,
            ...
            polizLexemsGrid.Columns.Add("Lex", "Лексема");
            polizLexemsGrid.Columns.Add("Kind", "Тип");
            polizLexemsGrid.Columns.Add("Pos", "Позиция");
            polizLexemsGrid.Columns["Lex"].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
            polizLexemsGrid.Columns["Kind"].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
            polizLexemsGrid.Columns["Pos"].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;

            polizTetradsGrid = new DataGridView
            {
                Dock = DockStyle.Fill,
                AllowUserToAddRows = false,
                AllowUserToDeleteRows = false,
            ...
            polizTetradsGrid.Columns.Add("Op", "op");
            polizTetradsGrid.Columns.Add("A1", "arg1");
            polizTetradsGrid.Columns.Add("A2", "arg2");
            polizTetradsGrid.Columns.Add("Res", "result");
            polizTetradsGrid.Columns["Op"].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
            polizTetradsGrid.Columns["A1"].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
            polizTetradsGrid.Columns["A2"].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
            polizTetradsGrid.Columns["Res"].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;

            tablesSplit.Panel1.Controls.Add(polizLexemsGrid);
            tablesSplit.Panel2.Controls.Add(polizTetradsGrid);

            polizSummaryRichTextBox = new RichTextBox
            {
                Dock = DockStyle.Fill,
                ReadOnly = true,
                WordWrap = true,
            ...
            outer.Panel1.Controls.Add(tablesSplit);
            outer.Panel2.Controls.Add(polizSummaryRichTextBox);
            polizTabPage.Controls.Add(outer);
            LowerTabs.Controls.Add(polizTabPage);
        }
        ...
        public bool IsPolizTabSelected()
        {
            return polizTabPage != null && LowerTabs.SelectedTab == polizTabPage;
        }

        public void FillPolizAnalysis(PolizAnalysisResult result)
        {
            if (polizLexemsGrid == null || polizTetradsGrid == null || polizSummaryRichTextBox == null || result == null)
            {
                return;
            }

            polizLexemsGrid.Rows.Clear();
            foreach (var row in result.LexemeRows)
            {
                polizLexemsGrid.Rows.Add(row.Lexeme, row.Kind, row.Position);
            }

            polizTetradsGrid.Rows.Clear();
            foreach (var t in result.Tetrads)
            {
                polizTetradsGrid.Rows.Add(t.Op, t.Arg1, t.Arg2, t.Result);
            }

            var sb = new System.Text.StringBuilder();
            foreach (var e in result.LexicalErrors)
            {
                sb.AppendLine("• " + e);
            }

            foreach (var e in result.SyntaxErrors)
            {
                sb.AppendLine("• " + e);
            }
            ...
            if (result.LexicalErrors.Count == 0 && result.SyntaxErrors.Count == 0)
            {
                sb.AppendLine("Синтаксический разбор выполнен успешно.");
                if (result.Tetrads.Count > 0)
                {
                    sb.AppendLine("Тетрады с временными переменными t1, t2, … — в правой таблице.");
                }

                if (!string.IsNullOrEmpty(result.Poliz))
                {
                    sb.AppendLine();
                    sb.AppendLine("ПОЛИЗ (алгоритм Дейкстры):");
                    sb.AppendLine(result.Poliz);
                }

                if (result.ComputedValue.HasValue)
                {
                    sb.AppendLine();
                    sb.AppendLine("Значение выражения: " + result.ComputedValue.Value);
                }
            }

            polizSummaryRichTextBox.Text = sb.ToString().TrimEnd();
        }
